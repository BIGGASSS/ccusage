...
Parallel results retain distinct request identities even when their usage is identical.
...
speculatively. Copied subagent tool results participate in replay matching as
individual expanded requests, including on tree branches.

Auto mode recalculates negative or non-finite source costs, while preserving
valid zero costs. Token-based pricing uses each request's timestamp, including
nested subagent timestamps; aggregate fallback uses the tool result timestamp.
