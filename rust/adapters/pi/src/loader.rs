...
mod tests {
    ...
    #[test]
    fn skips_copied_subagent_requests_and_aggregate_fallback_in_linear_history() {
        let fixture = Fixture::new();
        let history = format!(
            "{}\n{}",
            include_str!("fixtures/subagent-nested.jsonl").trim(),
            include_str!("fixtures/subagent-fallback.jsonl").trim(),
        );
        let parent = fixture.write_file(
            "sessions/project-a/root.jsonl",
            format!(
                "{}\n{history}",
                session_line("root", "2026-01-01T00:00:00Z", None)
            ),
        );
        let _ = fixture.write_file(
            ...
            format!(
                "{}\n{history}\n{}",
                session_line("child", "2026-01-06T00:00:00Z", Some(&parent)),
                usage_line("2026-01-07T00:00:00Z", 500, 10, 20, 3),
            ),
        );
        assert_subagent_replay_entries(&fixture, 5);
    }

    #[test]
    fn skips_all_expanded_subagent_requests_on_the_copied_active_tree_branch() {
        let fixture = Fixture::new();
        let linked_tool_result = |content: &str, id: &str, parent: &str| {
            let mut line: serde_json::Value = serde_json::from_str(content).unwrap();
            line["id"] = json!(id);
            line["parentId"] = json!(parent);
            line.to_string()
        };
        let nested = include_str!("fixtures/subagent-nested.jsonl")
            .lines()
            .nth(1)
            .unwrap();
        let fallback = include_str!("fixtures/subagent-fallback.jsonl")
            .lines()
            .next()
            .unwrap();
        let parent = fixture.write_file(
            ...
            [
                session_line("root", "2026-01-01T00:00:00Z", None),
                linked_usage_line("a", None, "2026-01-02T00:00:00Z", 100),
                linked_usage_line("abandoned", Some("a"), "2026-01-02T01:00:00Z", 200),
                linked_tool_result(nested, "nested", "a"),
                linked_tool_result(fallback, "fallback", "nested"),
                linked_usage_line("end", Some("fallback"), "2026-01-06T00:00:00Z", 300),
            ]
            .join("\n"),
        );
        let _ = fixture.write_file(
            ...
            [
                session_line("child", "2026-01-06T00:00:00Z", Some(&parent)),
                linked_usage_line("copy-a", None, "2026-01-02T00:00:00Z", 100),
                linked_tool_result(nested, "copy-nested", "copy-a"),
                linked_tool_result(fallback, "copy-fallback", "copy-nested"),
                linked_usage_line(
                    "copy-end",
                    Some("copy-fallback"),
                ...
                linked_usage_line("new", Some("copy-end"), "2026-01-07T00:00:00Z", 500),
            ]
            .join("\n"),
        );
        assert_subagent_replay_entries(&fixture, 7);
    }

    fn assert_subagent_replay_entries(fixture: &Fixture, parent_count: usize) {
        for single_thread in [true, false] {
            ...
            .unwrap();
            assert_eq!(entries.len(), parent_count + 1);
            let child = entries
                .iter()
                .filter(|entry| entry.session_id.as_ref() == "child")
                .collect::<Vec<_>>();
            assert_eq!(child.len(), 1);
            assert_eq!(child[0].data.message.usage.input_tokens, 500);
            let parallel = entries
                .iter()
                .filter(|entry| {
                    entry.model.as_deref() == Some("[pi] gpt-sub")
                        && entry.data.message.usage.input_tokens == 100
                })
                .collect::<Vec<_>>();
            assert_eq!(parallel.len(), 2);
            assert_ne!(parallel[0].data.request_id, parallel[1].data.request_id);
        }
    }
