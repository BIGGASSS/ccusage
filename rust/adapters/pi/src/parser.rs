...
fn replay_usage_path(content: &[u8], usage: &[PiUsageRecord]) -> PiReplayUsagePath {
    ...
    let mut usage_by_id: HashMap<&str, Vec<usize>> = HashMap::new();
    for (index, usage) in usage.iter().enumerate() {
        if let Some(id) = usage.entry_id.as_deref() {
            usage_by_id.entry(id).or_default().push(index);
        }
    }
    ...
    loop {
        ...
        if let Some(usage_indices) = usage_by_id.get(entry_id.as_str()) {
            // The whole path is reversed below, including requests within each row.
            active_usage.extend(usage_indices.iter().rev().copied());
        }
...
fn push_entry(
    ...
    usage_records: &mut Vec<PiUsageRecord>,
    entry_id: Option<&str>,
    project: &str,
    session_id: &str,
    ...
    // Replay prefixes are measured in expanded entries, not physical JSONL rows.
    // All requests in a tool result retain that row's tree link identity.
    usage_records.push(PiUsageRecord {
        signature: PiUsageSignature {
            timestamp,
            model: raw_model.map(str::to_owned),
...
mod tests {
    ...
    #[test]
    fn retains_model_namespace_after_subagent_provider_prefix() {
        assert_eq!(
            subagent_result_model("openrouter/anthropic/claude-opus-4.7"),
            "anthropic/claude-opus-4.7"
        );
        assert_eq!(subagent_result_model("provider/model"), "model");
        assert_eq!(subagent_result_model("model"), "model");
        assert_eq!(subagent_result_model("provider/"), "provider/");
    }
    ...
    #[test]
    fn applies_invalid_source_cost_policy_to_nested_and_aggregate_subagent_usage() {
        let mut pricing = PricingMap::default();
        pricing.load_json(
            r#"{"test-model":{"input_cost_per_token":0.000001,"output_cost_per_token":0.000002}}"#,
        );
        let cases = [
            (serde_json::json!(-1.25), 0.005, -1.25),
            (serde_json::json!(0), 0.0, 0.0),
            (serde_json::json!("bad"), 0.005, 0.0),
            (serde_json::Value::Null, 0.005, 0.0),
            (serde_json::json!({}), 0.005, 0.0),
        ];
        for (source_cost, auto_cost, display_cost) in cases {
            let fixture = fs_fixture!({});
            let content = serde_json::json!({
                "type": "message",
                "timestamp": "2026-01-03T00:00:00Z",
                "message": {
                    "role": "toolResult", "toolName": "subagent", "toolCallId": "cost-test",
                    "details": {"results": [
                        {"model": "provider/test-model", "usage": {"input": 999999, "cost": 9},
                         "messages": [{"role": "assistant", "timestamp": 1767398400000_i64,
                            "usage": {"input": 1000, "output": 2000, "cost": {"total": source_cost}}}]},
                        {"model": "provider/test-model", "usage": {"input": 1000, "output": 2000, "cost": source_cost}}
                    ]}
                }
            });
            let file = fixture_file(&fixture, "agent_costs.jsonl", &content.to_string());
            for context in [
                PiStoreContext::Default,
                PiStoreContext::Named {
                    root: &fixture.path("sessions"),
                    name: "omp",
                },
            ] {
                for (mode, expected) in [
                    (CostMode::Auto, auto_cost),
                    (CostMode::Display, display_cost),
                    (CostMode::Calculate, 0.005),
                ] {
                    let data = read_session_file_data_with_context(
                        &file,
                        None,
                        mode,
                        Some(&pricing),
    ...
    #[test]
    fn prices_nested_requests_at_their_own_timestamp_and_aggregate_at_tool_timestamp() {
        let fixture = fs_fixture!({});
        let nested_timestamp = "2026-08-17T01:00:00Z"
            .parse::<jiff::Timestamp>()
            .unwrap()
            .as_millisecond();
        let content = serde_json::json!({
            "type": "message", "timestamp": "2026-08-17T04:00:00Z",
            "message": {
                "role": "toolResult", "toolName": "subagent", "toolCallId": "time-test",
                "details": {"results": [
                    {"model": "deepseek/deepseek-v4-flash", "messages": [
                        {"role": "assistant", "timestamp": nested_timestamp, "usage": {"input": 1000000}}
                    ]},
                    {"model": "deepseek/deepseek-v4-flash", "usage": {"input": 1000000}}
                ]}
            }
        });
        let file = fixture_file(&fixture, "agent_pricing.jsonl", &content.to_string());
        let mut pricing = PricingMap::default();
        pricing.load_json(r#"{"deepseek-v4-flash":{"input_cost_per_token":0.00000014,"output_cost_per_token":0.00000028}}"#);
        for context in [
            PiStoreContext::Default,
            PiStoreContext::Named {
                root: &fixture.path("sessions"),
            ...
            for mode in [CostMode::Auto, CostMode::Calculate] {
                let data =
                    read_session_file_data_with_context(&file, None, mode, Some(&pricing), context)
                        .unwrap();
                assert_eq!(data.entries.len(), 2);
                assert!((data.entries[0].cost - 0.44).abs() < 1e-12);
                assert!((data.entries[1].cost - 0.22).abs() < 1e-12);
            }
        }
    }
